...
Covers:
    ...
    - Forward projections at 1yr/5yr/10yr for revenue, net income, and
      Net Debt/EBITDA (trailing growth rate compounded, margin/debt
...
def compute_forward_projections(revenue: float | None, ebitda: float | None,
    ...
    next to the projection table, never bury them):
        ...
        3. Revenue growth continues at the trailing YoY rate at every
           horizon (no deceleration/acceleration/business-model-transition
           modeled).
    ...
    YoY today implies an absurd 4x revenue figure at 10yr under this model.
    This module marks which horizons are "long" via LONG_HORIZONS so the
    UI layer can apply that stronger caveat consistently; the caveat text
    itself is a UI-layer concern, not rendered here.
    ...
    if net_debt is None or ebitda in (None, 0) or revenue_growth_yoy is None:
    ...
    for h in horizons:
        growth_factor = (1 + revenue_growth_yoy) ** h
